    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                $(this.modal_selector).bind('input', '[data-unique]', function(elm) {
                    ...
                    for (var j in colData) {
                        // if the element is in the column and its not the
						// selected one then its not unique
            ...
            _openEditModal: function () {
                ...
                for (var j in columnDefs) {
                    if (columnDefs[j].name != null) {
                        ...
                        $(selector).find(jquerySelector).trigger("change"); // required
																			// by
																			// select2
                         // added checkbox
                        if (columnDefs[j].type.indexOf("checkbox") >= 0) {
                            if (this._quoteattr(selectedValue) === "true") {
                                $(selector).find(jquerySelector).prop("checked", this._quoteattr(selectedValue)); // required
																													// by
																													// checkbox
                            }
                        }
            ...
            _openDeleteModal: function () {
				...
				// this.language.delete.title, this.language.delete.button,
                // this.language.modalClose, 'deleteRowBtn',
                ...
                for (var j in columnDefs) {
                    if (columnDefs[j].type.indexOf("hidden") >= 0 ||  columnDefs[j].type.indexOf("password") >= 0) {
            ...
            _deleteRow: function () {
                ...
                for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
                    // .data is the attribute name, if any; .idx is the column
					// index, so it should always exists
            ...
            completeColumnDefs: function () {
                ...
                for (var i in dt.context[0].aoColumns) {
                    ...
                    columnDefs[i] = {
                        ...
                        msg: (obj.errorMsg ? obj.errorMsg : ''),        // FIXME
																		// no
																		// more
																		// used
                        ...
                        uniqueMsg: (obj.uniqueMsg ? obj.uniqueMsg : ''),        // FIXME
																				// no
																				// more
																				// used
			 ...
			 * 
			 * @param columnDefs
			 *            as returned by completeColumnDefs()
			 */
                ...
                for (var j in columnDefs) {
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        if (columnDefs[j].type.indexOf("readonly") >= 0) {
                            // type=readonly is deprecated, kept for backward
							// compatibility
                        ...
                        // Adding text-inputs and errorlabels, but also new
						// HTML5 typees (email, color, ...)
                        else {
