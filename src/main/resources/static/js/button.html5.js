	...
	if (typeof define === "function" && define.amd) {
		...
		(function($, window, document, undefined) {
			...
			var _saveAs = (function(view) {
				...
				// only get URL when necessary in case Blob.js hasn't overridden
				// it yet
				get_URL = function() {
					return view.URL || view.webkitURL || view;
				}, save_link = doc.createElementNS(
						"http://www.w3.org/1999/xhtml", "a"), can_use_save_link = "download" in save_link, click = function(
						node) {
				...
				revoke = function(file) {
					...
					// prepend BOM for UTF-8 XML and text/* types (including
					// HTML)
					...
					blob = auto_bom(blob);
					// First try a.download, then web filesystem, then object
					// URLs
					...
					// Update: Google errantly closed 91158, I submitted it
					// again:
			 ...
			 * @param {boolean}
			 *            incExtension Include the file name extension
			 */
			var _filename = function(config, incExtension) {
				...
				return incExtension === undefined || incExtension === true ? filename
						+ config.extension
						: filename;
			};
			...
			var _title = function(config) {
				...
				return title.indexOf("*") !== -1 ? title.replace("*",
						$("title").text()) : title;
			};
			...
			/**
			 * Combine the data from the `buttons.exportData` method into a
			 * string that will be used in the export file.
			 * 
			 * @param {DataTable.Api}
			...
			var _exportData = function(dt, config) {
				...
				var join = function(a) {
					...
					// If there is a field boundary, then we might need to
					// escape it in
					// the source data
					for (var i = 0, ien = a.length; i < ien; i++) {
						...
						s += boundary ? boundary
								+ ("" + a[i]).replace(reBoundary, escapeChar
										+ boundary) + boundary : a[i];
					}
				...
				var footer = config.footer && data.footer ? newLine
						+ join(data.footer) : "";
				var body = [];
			...
			/**
			 * Safari's data: support for creating and downloading files is
			 * really poor, so various options need to be disabled in it. See
			...
			var _isSafari = function() {
				return (navigator.userAgent.indexOf("Safari") !== -1
						&& navigator.userAgent.indexOf("Chrome") === -1 && navigator.userAgent
						.indexOf("Opera") === -1);
			};
		...
		__DATA__\
			...
			DataTable.ext.buttons.copyHtml5 = {
				...
				action : function(e, dt, button, config) {
					...
					// For browsers that support the copy execCommand, try to
					// use it
					if (document.queryCommandSupported("copy")) {
						...
						try {
							...
							dt.buttons.info(dt.i18n("buttons.copyTitle",
									"Copy to clipboard"), dt.i18n(
									"buttons.copySuccess", {
					...
					// Otherwise we show the text box and instruct the user to
					// use it
					var message = $(
							"<span>"
									+ dt
									...
									+ "</span>").append(hiddenDiv);

					dt.buttons.info(dt.i18n("buttons.copyTitle",
							"Copy to clipboard"), message, 0);
			...
			DataTable.ext.buttons.excelHtml5 = {
				...
				available : function() {
					return (window.FileReader !== undefined
							&& window.JSZip !== undefined && !_isSafari());
				},
				...
				action : function(e, dt, button, config) {
					...
					var addRow = function(row) {
						...
						try {
							for (var i = 0, ien = row.length; i < ien; i++) {
								...
								// Don't match numbers with leading zeros or a
								// negative anywhere
								// but the start
								cells
										.push(typeof row[i] === "number"
												|| (row[i].match
														&& $
																.trim(row[i])
																.match(
					...
					// Load Report headers
					var get_title = window.localStorage
							.getItem("report_headers");
					...
					/*
					 * CG - Implement Multiple Footer Excel
					 */
					if (config.customFooter === true) {
						var _get_excel_footer = window.localStorage
								.getItem("excelFooter");
						var _write_excel_footer = JSON.parse(_get_excel_footer);
						_write_excel_footer.forEach(function(excelData) {
							xml += addRow(excelData);
						});
					}
					...
					xl.file("workbook.xml", excelStrings["xl/workbook.xml"]
							.replace("__SHEET_NAME__", _sheetname(config)));
					xl_rels.file("workbook.xml.rels",
							excelStrings["xl/_rels/workbook.xml.rels"]);
					xl_worksheets.file("sheet1.xml",
							excelStrings["xl/worksheets/sheet1.xml"].replace(
									"__DATA__", xml));
